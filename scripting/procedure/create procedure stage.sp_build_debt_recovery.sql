...
BEGIN
  ...
  SELECT
    ...
    WHERE H2.NEGOTIATOR_KEY = H.NEGOTIATOR_KEY AND LEFT(P.PAYMENT_DATE,6) = H2.PERIOD_CODE
      AND EXISTS
      (
        SELECT
          1
        ...
        WHERE
        P2.PAYMENT_DATE = P.PAYMENT_DATE AND P2.CREDIT_KEY = H2.CREDIT_KEY AND P2.CURRENCY_KEY = P.CURRENCY_KEY
      )
    ) AS CREDIT_RECOVERED,
    (
    SELECT 
  ...
  ON LEFT(P.PAYMENT_DATE,6) = H.PERIOD_CODE
    AND P.CREDIT_KEY = H.CREDIT_KEY
   GROUP BY  H.NEGOTIATOR_KEY, P.PAYMENT_DATE, P.CURRENCY_KEY
